...
<div class="w-full min-h-screen bg-[#fefbf4]">
    ...
    <div id="latestNew" class="w-full flex justify-center pt-10" 
        style="display: <?= $isSectionVisible('latestNew') ? 'flex' : 'none' ?>; font-family: 'Roboto Mono', monospace;">
        <div class="w-[90%] lg:w-[90%]">
            <!-- heading --> 
            <div class="w-full flex items-center justify-between py-6">
                <p class="text-xl md:text-5xl text-[#1f1c17] italic font-bold">
                    Bài viết mới nhất
                </p>
                    ...
                    <?php foreach ($latestPosts as $post): ?>
                        <div class="w-full lg:w-1/3 flex flex-col items-stretch justify-start gap-2">
                            <a href="<?= SITE_URL ?? '' ?>/post/view/<?= $post['id'] ?>"
                                class="hover:shadow hover:scale-105 transition-transform duration-200">
                                <div class="w-full overflow-hidden" style="height: 350px;">
                                    <img
                                        src="<?= (SITE_URL ?? '') . htmlspecialchars($post['thumbnail_url'] ?? '') ?>"
                                        class="block w-full"
                                        style="height: 350px; object-fit: cover;"
                                        alt="<?= htmlspecialchars($post['title'] ?? '') ?>" />
                                </div>
                                <div class="flex flex-col gap-3 w-full">
                                    <p class="text-sm font-semibold text-[#C5A25D]">
                                        <?= date('d.m.Y', strtotime($post['updated_at'])) ?>
                                    </p>
                                    <p class="text-[#1f1c17] font-medium">
                                    ...
                                    <p class="overflow-hidden"
                                        ...
                                        <?= htmlspecialchars($post['thumbnail_description'] ?? '') ?>
                                    </p>
